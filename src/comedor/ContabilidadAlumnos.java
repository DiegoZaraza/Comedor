...
public class ContabilidadAlumnos extends JInternalFrame implements Runnable {
	...
	public void paintComponent(Graphics g) {
		setOpaque(false);
		g.drawImage(new ImageIcon(getClass().getResource("/resource/e2.jpg"))
		.getImage(), 0, 0, getWidth(), getHeight(), null);
	...
	public ContabilidadAlumnos(ComedorGUI gui) {
			...
			public void paintComponent(Graphics g) {
				...
				g.drawImage(
						new ImageIcon(getClass()
								.getResource("/resource/e2.jpg")).getImage(),
		...
		gl_panel.setHorizontalGroup(gl_panel.createParallelGroup(
				Alignment.LEADING).addGap(0, 696, Short.MAX_VALUE));
		...
		table.addMouseListener(new MouseListener() {
			...
			public void mouseReleased(MouseEvent arg0) {
				...
				principal.getBaseDeDatos().getFechasPeriodo(
						table.getValueAt(table.getSelectedRow(), 1).toString(),
						tableFechas, listFechas);
			}
		...
		groupLayout.setHorizontalGroup(
				...
				.addGroup(groupLayout.createSequentialGroup()
						.addGroup(groupLayout.createParallelGroup(Alignment.LEADING)
								.addComponent(panel_8, GroupLayout.PREFERRED_SIZE, 839, Short.MAX_VALUE)
								.addGroup(Alignment.TRAILING, groupLayout.createSequentialGroup()
										.addComponent(panel, GroupLayout.PREFERRED_SIZE, 329, GroupLayout.PREFERRED_SIZE)
										.addPreferredGap(ComponentPlacement.RELATED)
										.addComponent(panel_10, GroupLayout.DEFAULT_SIZE, 504, Short.MAX_VALUE)))
										.addContainerGap())
				);
		...
		JButton btnRemesasMensuales = new JButton("Remesas Mensuales");

		JProgressBar progressBarRemesas = new JProgressBar();
		progressBarRemesas.setStringPainted(true);
		progressBarRemesas.setPreferredSize(new Dimension(280, 24));
		progressBarRemesas.setForeground(Color.ORANGE);
		panel_10.add(progressBarRemesas);

		btnRemesasMensuales.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				String curso = principal.getBaseDeDatos().getCursoActual();
				ArrayList<Persona> alumnos = principal.getBaseDeDatos().getTodosLosAlumnos(curso);
				...
				new Thread(
						new Runnable()
						{
							...
							public void run()
							{
								try
								{
									progressBarRemesas.setString("Generando Remesas...");
									progressBarRemesas.setIndeterminate(true);
									...
									while(true)
									{
										...
										{
											progressBarRemesas.setString("Generando Remesas (" + i + ")");
											principal.getBaseDeDatos().getRemesasMasivas(alumnos.get(i).getNia());
										}
										Thread.sleep(500);

										progressBarRemesas.setIndeterminate(false);
										progressBarRemesas.setString("Completado!");
										break;
									}
								}
								catch(Exception e)
				...
				//						System.out.println(listFechas.get(tableFechas.getSelectedRow()));
				//						principal.getBaseDeDatos().getRemesas(id, tableRemesas, arrayList);
				//						principal.getBaseDeDatos().getRemesasGeneradas(id, tableRemesas,
				//								arrayList, arrayListIDGeneradas, nRemesas);
			}
		});
		panel_10.add(btnRemesasMensuales);
		...
		panel_5.setBorder(new TitledBorder(UIManager
				.getBorder("TitledBorder.border"), "Remesa Seleccionada",
		...
		comboBox.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				if (comboBox.getSelectedItem().toString()
						.equalsIgnoreCase("PAGADO CON COMISION")) 
			...
			public void actionPerformed(ActionEvent arg0) {
				if (comboBox.getSelectedItem().toString()
					...
					if (tableRemesas.getSelectedRow() != -1) {
						principal.getBaseDeDatos().updateRemesa(
								arrayList.get(tableRemesas.getSelectedRow()),
								"PAGADO");
						String id = listFechas.get(tableFechas.getSelectedRow());

						principal.getBaseDeDatos().getRemesasGeneradas(id,
								tableRemesas, arrayList, arrayListIDGeneradas,
								nRemesas);
					} else {
						return;
					}
		...
		btnNewButton.setIcon(new ImageIcon(ContabilidadAlumnos.class
		...
		tableFechas.addMouseListener(new MouseListener() {
			...
			@Override
			public void mousePressed(MouseEvent arg0) {
				String nia = table.getValueAt(table.getSelectedRow(), 1)
						.toString();
				String id = listFechas.get(tableFechas.getSelectedRow());

				principal.getBaseDeDatos()
				.getRemesasGeneradas(id, tableRemesas, arrayList,
						arrayListIDGeneradas, nRemesas);

				principal.getBaseDeDatos().getDevoluciones(tableDevoluciones,
						arrayListDevoluciones, nia, id,
						arrayListIDRemesasDevoluciones);
			}
			@Override
			public void mouseExited(MouseEvent arg0) {
			}
		...
		panel_7.setBorder(new TitledBorder(null, "Nueva Devolucion",
				TitledBorder.LEADING, TitledBorder.TOP, null, null));
		...
		btnNewButton_2.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				if (motivo.getText().length() == 0) {
					JOptionPane.showMessageDialog(principal,
							"Debe especificar un motivo!", "Motivo",
							JOptionPane.WARNING_MESSAGE);
					return;
				}

				int c = JOptionPane.showConfirmDialog(principal,
						"Esta seguro(a) de aplicar esta Devolucion?",
						"Confirmacion", JOptionPane.OK_OPTION);

				if (c == JOptionPane.OK_OPTION) {
					String nia = table.getValueAt(table.getSelectedRow(), 1)
							.toString();
					String id = listFechas.get(tableFechas.getSelectedRow());
					principal.getBaseDeDatos().addDevolucion(
							nia,
							nRemesas.getSelectedItem().toString(),
							arrayListIDGeneradas.get(nRemesas
									.getSelectedIndex()), motivo.getText(),
									observacion.getText(),
									porcentaje.getSelectedItem().toString(), id);
					principal.getBaseDeDatos().getDevoluciones(
							tableDevoluciones, arrayListDevoluciones, nia, id,
							arrayListIDRemesasDevoluciones);
					principal.getBaseDeDatos().getRemesasGeneradas(id,
							tableRemesas, arrayList, arrayListIDGeneradas,
							nRemesas);
		...
		btnNewButton_2.setIcon(new ImageIcon(ContabilidadAlumnos.class
				.getResource("/resource/n41.png")));
		...
		btnCerrar.setIcon(new ImageIcon(ContabilidadAlumnos.class
				.getResource("/resource/close1.png")));
		...
		btnNewButton_3.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				if (tableDevoluciones.getSelectedRow() != -1) {
					String id = arrayListIDRemesasDevoluciones
							.get(tableDevoluciones.getSelectedRow());
					int l = JOptionPane
							.showConfirmDialog(
									principal,
									"Esta seguro(a) de generar esta carta de Cobro para la Devolucion seleccionada?",
									"Confirmacion",
					...
					if (l == JOptionPane.OK_OPTION) {
						...
						String h = principal.getBaseDeDatos().getUltimaRemesa(
								nia);
						...
						principal.getBaseDeDatos().getEstadoCartas(id,
								tableCartas, nia);
					}
				} else {
					JOptionPane.showMessageDialog(principal,
							"Debe seleccionar una devolucion",
							"Seleccion vacia", JOptionPane.WARNING_MESSAGE);
					return;
				}
			}
		});
		...
		tableDevoluciones.addMouseListener(new MouseListener() {
			...
			public void mouseReleased(MouseEvent arg0) {
				if (tableDevoluciones.getSelectedRow() != -1) {
					...
					String nia = table.getValueAt(table.getSelectedRow(), 1)
							.toString();
					principal.getBaseDeDatos().getEstadoCartas(id, tableCartas,
							nia);
				}
			}
	...
	public void run() {
		...
		ArrayList<Persona> xz = principal.getBaseDeDatos().getTodosLosAlumnos(
				cursoActual);

		for (int index = 0; index < xz.size(); index++)
			table.addRow(xz.get(index).getNombreCompleto(), xz.get(index)
					.getNia());
	}
}