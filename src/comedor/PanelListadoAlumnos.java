...
public class PanelListadoAlumnos extends JPanel implements Runnable{
	/**
	 * 
	 */
...
//				principal.getBaseDeDatos().buscarAlumnos(field.getText(),jkTable,jLabel);
...
//		jkTable.setFont(new Font("arial",Font.PLAIN,13));
		...
		jkTable.setRowHeight(15);
		
//		jkTable.getColumn("Foto").setPreferredWidth(3);
//		jkTable.getColumn("Foto").setWidth(3);
	
		jkTable.getColumn("NIA").setPreferredWidth(8);
		jkTable.getColumn("NIA").setWidth(8);
		...
		JMenuItem itemcOPY = new JMenuItem("Copiar NIA");
		...
		itemcOPY.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				if(jkTable.getSelectedRow()!=-1){
					...
					String nia=jkTable.getValueAt(jkTable.getSelectedRow(), 0).toString();
	...
	@Override
	public void run() {
		principal.getBaseDeDatos().buscarAlumnos("",jkTable,jLabel);

		
	}

	public void buscar(String fieldNombre) {
		jkTable.search(fieldNombre.toUpperCase());
