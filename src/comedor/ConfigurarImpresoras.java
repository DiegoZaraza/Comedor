...
public class ConfigurarImpresoras extends JInternalFrame 
{
	...
	public ConfigurarImpresoras(ComedorGUI comedorGUI) 
	{
		...
		if(x.size() == 0)
		{
			JOptionPane.showMessageDialog(principal, "No Existe Impresora Para Configurar", "Advertencia!", JOptionPane.WARNING_MESSAGE);
			dispose();
		}
		else
		{	
		...
		{
			if(comboBox.getItemAt(index).toString().equalsIgnoreCase(principal.getBaseDeDatos().getImpresoraCarnet()))
				comboBox.setSelectedIndex(index);
		}

		for(int index=0;index<comboBox_1.getItemCount();index++)
		{
			if(comboBox_1.getItemAt(index).toString().equalsIgnoreCase(principal.getBaseDeDatos().getImpresoraEstandar()))
