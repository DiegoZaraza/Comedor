...
public class PDF_Recibos1_Profesores 
{
	...
	public PDF_Recibos1_Profesores(ArrayList<String> arrayList, ComedorGUI principal) throws FileNotFoundException, DocumentException 
	{
		...
		PdfWriter.getInstance(document, new FileOutputStream("System-Comedor" + File.separator + "Temp" + File.separator + "listado_recibos.pdf"));
		document.open();

		Paragraph texto22 = new Paragraph("LISTADO POR RECIBO", FontFactory.getFont("arial",12, Font.BOLD, BaseColor.BLACK));
		...
		int tam=7;
		celda = new PdfPCell(new Paragraph("Apelidos y Nombre", FontFactory.getFont("arial", 
				tam, 
				Font.NORMAL, 
				BaseColor.BLACK))); 
		...
		celda = new PdfPCell(new Paragraph("Dias Ordinarios", FontFactory.getFont("arial", 
				tam, 
				Font.NORMAL, 
				BaseColor.BLACK))); 
		celda.setColspan(1);// abarca 1 fila
		...
		celda.setBackgroundColor(new BaseColor(new Color(230, 223, 223)));
		tabla.addCell(celda);
		celda = new PdfPCell(new Paragraph("Dias Extraordinarios", FontFactory.getFont("arial", 
				6, 
				Font.NORMAL, 
				BaseColor.BLACK))); 
		celda.setColspan(1);// abarca 1 fila
		...
		celda.setBackgroundColor(new BaseColor(new Color(230, 223, 223)));
		tabla.addCell(celda);
		celda = new PdfPCell(new Paragraph("Deusa Pendiente", FontFactory.getFont("arial", 
				6, 
				Font.NORMAL, 
				BaseColor.BLACK))); 
		celda.setColspan(1);// abarca 1 fila
		...
		for(int index=0;index<arrayList.size();index++){
			...
			t2 = new Paragraph(""+nombre, FontFactory.getFont("arial", 
					11, 
					Font.NORMAL, 
					BaseColor.BLACK));
			tabla.addCell(t2);
			sumaOrdinarios+=Integer.parseInt(diasOrdinarios);
			t2 = new Paragraph(diasOrdinarios, FontFactory.getFont("arial", 
					11, 
					Font.NORMAL, 
					BaseColor.BLACK));
			tabla.addCell(t2);
			sumaExtraOrdinarios+=Integer.parseInt(diasExtraOrdinarios);
			t2 = new Paragraph(diasExtraOrdinarios, FontFactory.getFont("arial", 
					11, 
					Font.NORMAL, 
					BaseColor.BLACK));
			tabla.addCell(t2);
			float m=Float.parseFloat(deuda);
			t2 = new Paragraph(""+df.format(m)+"€", FontFactory.getFont("arial", 
					11, 
					Font.NORMAL, 
					BaseColor.BLACK));
			tabla.addCell(t2);
		...
		uc1 = new PdfPCell(new Paragraph("Suma Total dias ExtraOrdinarios: " + (sumaExtraOrdinarios) + "  IMPORTE:[ " + df.format((sumaExtraOrdinarios * c)) + "€ ]", FontFactory.getFont("arial", 8, Font.BOLD, BaseColor.BLACK))); 
		uc1.setColspan(1);// abarca 1 fila
		uc1.setHorizontalAlignment(Element.ALIGN_CENTER);
		uc1.setVerticalAlignment(Element.ALIGN_CENTER);
		...
		if(principal.getBaseDeDatos().getImpresionDirecta())
			new Impresora(principal).print(new File("System-Comedor" + File.separator + "Temp" + File.separator + "listado_recibos.pdf").toString());
		else
		{
			try 
			{
				Desktop.getDesktop().open(new File("System-Comedor" + File.separator + "Temp" + File.separator + "listado_recibos.pdf"));
			}
			catch (IOException e) 
			{
				e.printStackTrace();
