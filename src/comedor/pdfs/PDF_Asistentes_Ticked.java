...
public class PDF_Asistentes_Ticked {

	private static Document document;
	private static PdfPCell celda;
	private static Paragraph t2;

	public PDF_Asistentes_Ticked(ArrayList<Persona> arrayList, java.util.Date date,
			java.util.Date date2,ComedorGUI principal) throws FileNotFoundException, DocumentException {
		// TODO Auto-generated constructor stub
		// TODO Auto-generated method stub
				document = new Document();
				PdfWriter.getInstance(document,
						new FileOutputStream("System-Comedor"+File.separator+"Temp"+File.separator+"listado_asistentes_ticked.pdf"));
				document.open();
				Paragraph texto22 = new Paragraph("LISTADO DE ASISTENTES AL COMEDOR CON TICKED", FontFactory.getFont("arial",12,
...
//				celda.set
				...
				for(int index=0;index<arrayList.size();index++){
					...
					t2 = new Paragraph(p.getApellido1()+" "+p.getApellido2()+", "+p.getNombres(), FontFactory.getFont("arial", // fuente
							...
							BaseColor.BLACK));
					tabla.addCell(t2);
					t2 = new Paragraph(""+principal.getBaseDeDatos().getFechaHumana(p.getFecha()), FontFactory.getFont("arial", // fuente
							11, // tamaño
							Font.NORMAL, // estilo
							BaseColor.BLACK));
					count+=+p.getFaltas();
				...
				PdfPCell uc1 = new PdfPCell(new Paragraph("Asistencias Totales: "+count, FontFactory.getFont(
						"arial", // fuente
						8, // tamaño
						Font.BOLD, // estilo
						BaseColor.BLACK))); // color
...
//				tablatotal.addCell("");
				...
				if(principal.getBaseDeDatos().getImpresionDirecta()){
					  new Impresora(principal).print(new File("System-Comedor"+File.separator+"Temp"+File.separator+"listado_asistentes_ticked.pdf").toString());
				}else{
					try {
						Desktop.getDesktop().open(
								new File("System-Comedor"+File.separator+"Temp"+File.separator+"listado_asistentes_ticked.pdf"));
					} catch (IOException e) {
