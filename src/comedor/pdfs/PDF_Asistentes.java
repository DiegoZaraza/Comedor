...
public class PDF_Asistentes 
{
	private static Document document;
	private static PdfPCell celda;
	private static Paragraph t2;

	public PDF_Asistentes(ArrayList<Persona> arrayList, java.util.Date date, java.util.Date date2, ComedorGUI principal) throws FileNotFoundException, DocumentException 
	{
		document = new Document();
		PdfWriter.getInstance(document, new FileOutputStream("System-Comedor" + File.separator + "Temp" + File.separator + "listado_asistentes.pdf"));
		document.open();
		Paragraph texto22 = new Paragraph("LISTADO DE ASISTENTES AL COMEDOR", FontFactory.getFont("arial", 12, Font.NORMAL, BaseColor.BLACK));
		...
		Paragraph texto212 = new Paragraph("Desde: " + fecha1 + " Hasta:" + fecha2 + "", FontFactory.getFont("arial", 9, Font.NORMAL, BaseColor.BLACK));
		...
		celda = new PdfPCell(new Paragraph("ASISTENCIAS", FontFactory.getFont("arial", 6, Font.NORMAL, BaseColor.BLACK))); 
		celda.setColspan(1);
		celda.setHorizontalAlignment(Element.ALIGN_CENTER);
		...
		for (int index = 0; index < arrayList.size(); index++) 
		{
			...
			t2 = new Paragraph(p.getApellido1() + " " + p.getApellido2() + ", " + p.getNombres(), FontFactory.getFont("arial", 11, Font.NORMAL, BaseColor.BLACK));
			tabla.addCell(t2);
			
			t2 = new Paragraph("" + p.getFaltas(), FontFactory.getFont("arial", 11, Font.NORMAL, BaseColor.BLACK));
			count += +p.getFaltas();
		...
		PdfPCell uc1 = new PdfPCell(new Paragraph("Asistencias Totales: " + count, FontFactory.getFont("arial", 12, Font.BOLD, BaseColor.BLACK))); 
		uc1.setColspan(1);
		...
		if (principal.getBaseDeDatos().getImpresionDirecta()) 
			new Impresora(principal).print(new File("System-Comedor" + File.separator + "Temp" + File.separator + "listado_asistentes.pdf").toString());
		else
		{
			try 
			{
				Desktop.getDesktop().open( new File("System-Comedor" + File.separator + "Temp" + File.separator + "listado_asistentes.pdf"));
			} 
			catch (IOException e) 
			{
				e.printStackTrace();
