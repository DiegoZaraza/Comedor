...
public class PDF_RecibosPendientes {
	...
	public PDF_RecibosPendientes(ArrayList<Persona> arrayList, ComedorGUI principal) throws FileNotFoundException, DocumentException {
		...
		// TODO Auto-generated method stub
				...
				document.open();
				Paragraph texto22 = new Paragraph("estado de recibos pendientes de cobro".toUpperCase(), FontFactory.getFont("arial",12,
...
//				
				...
				tabla.addCell(celda);
				celda = new PdfPCell(new Paragraph("Debe", FontFactory.getFont("arial", // fuente
						tam, // tamaño
						Font.NORMAL, // estilo
						BaseColor.BLACK))); // color
				celda.setColspan(1);// abarca 1 fila
				...
				for(int index=0;index<arrayList.size();index++){
					...
					t2 = new Paragraph(""+p.getNombreCompleto(), FontFactory.getFont("arial", // fuente
							11, // tamaño
							Font.NORMAL, // estilo
							BaseColor.BLACK));
					tabla.addCell(t2);
					...
					t2 = new Paragraph(df.format(p.getDeuda())+"€", FontFactory.getFont("arial", // fuente
							11, // tamaño
							Font.NORMAL, // estilo
							BaseColor.BLACK));
					tabla.addCell(t2);
				...
				if(principal.getBaseDeDatos().getImpresionDirecta()){
					...
					try {
						Desktop.getDesktop().open(
								new File("System-Comedor"+File.separator+"Temp"+File.separator+"listado_pendientes.pdf"));
					} catch (IOException e) {
