...
public class PDF_Listado {
	...
	public boolean crearPDF5(ComedorGUI gui, String curso, String codigoGrupo,
			String nombreGrupo, String tutor, ArrayList<Persona> alumnos,
		...
		PdfWriter writer = PdfWriter.getInstance(document,
				new FileOutputStream("System-Comedor" + File.separator + "Temp"
						+ File.separator + tutor + ".pdf"));
		...
		if (principal.getBaseDeDatos().getImpresionDirecta())
			new Impresora(principal)
					.print(new File("System-Comedor" + File.separator + "Temp"
							+ File.separator + tutor + ".pdf").toString());
		else
			Desktop.getDesktop().open(
					new File("System-Comedor" + File.separator + "Temp"
							+ File.separator + tutor + ".pdf"));
	...
	/**
	 * AJUSTES PARA LA GENERACION DE ORLAS, SE CAMBIA A USO DE TABLA PARA QUE
	 * ESTAS NO SALGAN DESCUADRADAS COMO ESTABA OCURRIENDO ANTERIORMENTE
	 */
		...
		try {
			for (int i = 0; i < j; i++) {
				...
				if (nb.getBytesFoto() == null)
					foto = Image.getInstance(getClass().getResource(
							"/resource/default1.gif"));
				else
				...
				Paragraph nia = new Paragraph(nb.getNia(), FontFactory.getFont(
						"arial", 8, Font.BOLD, BaseColor.BLACK));
				Paragraph apellidos = new Paragraph(nb.getApellidos(),
						FontFactory.getFont("arial", 5, Font.NORMAL,
								BaseColor.BLACK));
				Paragraph nombre = new Paragraph(nb.getNombres(),
						FontFactory.getFont("arial", 5, Font.NORMAL,
								BaseColor.BLACK));
	...
	/**
	 * SE AGREGA ENCABEZADO Y PIE DE PAGINA PARA LA IMPRESION EN TODAS LAS HOJAS
	 * DEL DOCUMENTO
	 */
	public class FooterPiePaginaiText extends PdfPageEventHelper {
		...
		public void onStartPage(PdfWriter writer, Document document) {
			Paragraph texto1 = new Paragraph(""
					+ principal.getBaseDeDatos().getCentro(),
					FontFactory
							.getFont("arial", 14, Font.BOLD, BaseColor.BLACK));
			Paragraph texto21 = new Paragraph("" + curso, FontFactory.getFont(
					"arial", 9, Font.NORMAL, BaseColor.BLACK));
			Paragraph texto22 = new Paragraph(""
					+ principal.getBaseDeDatos().getCodigoCentro(),
			...
			if (!f)
				a11 = new Paragraph("TUTOR/A: " + tutor, FontFactory.getFont(
						"tahoma", 10, Font.NORMAL, BaseColor.BLACK));
			else
				a11 = new Paragraph(tutor, FontFactory.getFont("tahoma", 10,
			...
			ColumnText.showTextAligned(writer.getDirectContent(),
					Element.ALIGN_CENTER, texto21,
					document.right() - document.leftMargin(),
					document.top() - 20, 0);
			ColumnText.showTextAligned(writer.getDirectContent(),
					Element.ALIGN_CENTER, texto22,
					document.right() - document.leftMargin(),
					document.top() - 35, 0);
			ColumnText.showTextAligned(
					writer.getDirectContent(),
					...
					a11,
					(document.right() - document.left()) / 2
							+ document.leftMargin(), document.top() - 35, 0);
		}
		...
		public void onEndPage(PdfWriter writer, Document document) {
			ColumnText.showTextAligned(
					writer.getDirectContent(),
					Element.ALIGN_CENTER,
					new Phrase("Pagina : " + nPage, FontFactory.getFont(
							"arial", 9, Font.NORMAL, BaseColor.BLACK)), 55, 20,
					0);
			nPage++;
		}
	}
