...
//						 System.out.println(nb.getBytesFoto().length);
	...
	public boolean crearPDF5(ComedorGUI gui,String curso,String codigoGrupo,String nombreGrupo,String tutor,ArrayList<Persona> alumnos,boolean f) throws Exception {
		...
		this.f=f;
		// color
		PdfWriter writer = PdfWriter.getInstance(document, new FileOutputStream("System-Comedor"+File.separator+"listado.pdf"));
...
//		 
		...
		if(principal.getBaseDeDatos().getImpresionDirecta()){
			  new Impresora(principal).print(new File("System-Comedor"+File.separator+"listado.pdf").toString());
		}else{
		Desktop.getDesktop().open(
				new File("System-Comedor"+File.separator+"listado.pdf"));
		}
		return true;
	}
