...
public class ContabilidadProfesores extends JInternalFrame
{
	...
	public ContabilidadProfesores(ComedorGUI gui)
	{
		...
		gl_panel.setHorizontalGroup(gl_panel.createParallelGroup(Alignment.TRAILING).addGroup(gl_panel.createSequentialGroup().addContainerGap().addComponent(panel_1, GroupLayout.DEFAULT_SIZE, 807, Short.MAX_VALUE).addContainerGap())
		        .addComponent(tabbedPane, GroupLayout.PREFERRED_SIZE, GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE));

		gl_panel.setVerticalGroup(gl_panel.createParallelGroup(Alignment.LEADING).addGroup(
		        Alignment.TRAILING,
		        gl_panel.createSequentialGroup().addGap(5).addComponent(tabbedPane, GroupLayout.DEFAULT_SIZE, 460, Short.MAX_VALUE).addPreferredGap(ComponentPlacement.RELATED)
		                .addComponent(panel_1, GroupLayout.PREFERRED_SIZE, 44, GroupLayout.PREFERRED_SIZE).addContainerGap()));
		...
		btnNewButton_1.addActionListener(new ActionListener()
		{
			public void actionPerformed(ActionEvent arg0)
			{
				String TipoList[] = { "ORDINARIO", "EXTRAORDINARIO" };
				String bigList[] = { "01-ENERO", "02-FEBRERO", "04-MARZO", "04-ABRIL", "05-MAYO", "06-JUNIO", 
						"07-JULIO", "08-AGOSTO", "09-SEPTIEMBRE", "10-OCTUBRE", "11-NOVIEMBRE", "12-DICIEMBRE" };

				String tipo = JOptionPane.showInputDialog(principal, "Seleccionar Tipo Dias", "Seleccionar", JOptionPane.QUESTION_MESSAGE, null, TipoList, "Titan").toString();

				String selected = JOptionPane.showInputDialog(principal, "Seleccionar Mes", "Seleccionar", JOptionPane.QUESTION_MESSAGE, null, bigList, "Titan").toString();

				if (selected != null)
				{
					ArrayList<Persona> arrayList = principal.getBaseDeDatos().getProfesores(principal.getBaseDeDatos().getCursoActual());

					String nombre = "RegistroProfesores";

					new PDF_Formato_Apuntarse(principal, arrayList, selected, nombre, tipo);
				}
			}
		});
				...
				button2.addActionListener(new ActionListener()
				{
					...
					public void actionPerformed(ActionEvent arg0)
					{
						...
						if (selected != null)
						{
							...
							String nombre = "RegistroProfesoresSeleccionados";

							new PDF_Formato_Apuntarse(principal, arrayList1, selected, nombre, tipo);
						}
						dialog.dispose();
					}
				});
